...
class MyScene(LinearTransformationScene):
    ...
    def construct(self):
        ...
        # The scene itself
        vector_v_coords = np.array([2, 1])
        matrix = np.array([[-2, 2], [1, -3]])
        eigenvalue = -1

        # The value above is hardcoded, but we can actually check it here
        ...
        text = (
            MathTex(r"A\vec{v} = \begin{bmatrix}" + str(matrix[0][0]) + r"&" + str(matrix[0][1]) + r"\\" +
                    ...
                    str(eigenvalue) + r"\begin{bmatrix}" +
                    str(vector_v_coords[0]) + r"\\" + str(vector_v_coords[1]) + r"\end{bmatrix} = " +
        ...
        text[0][5:7].set_color(GREEN_C)
        text[0][8].set_color(GREEN_C)
        text[0][7].set_color(RED_C)
        text[0][9:11].set_color(RED_C)
        text[0][13:15].set_color(YELLOW)
        text[0][20:22].set_color(YELLOW)
        text[0][25:29].set_color(ORANGE)
        ...
        eigenline = DashedLine(np.array([-20, -10, 0]), np.array([20, 10, 0]), stroke_opacity=0.5, dash_length=0.15)
        self.add(eigenline)
