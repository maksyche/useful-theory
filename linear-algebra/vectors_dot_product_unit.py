...
class MyScene(Scene):
    def construct(self):
        ...
        vector_a_label = MathTex("\\hat{a}", color=RED).next_to(vector_a, direction=UP, buff=0.01)

        vector_b_coords = np.array([1, 0, 0])
        vector_b = plane.get_vector(np.array(vector_b_coords), color=BLUE)
        vector_b_label = MathTex("\\hat{b}", color=BLUE).next_to(vector_b, direction=DOWN, buff=0)

        alpha_tracker = ValueTracker(0)

        math_text = (MathTex("\\hat{a} \\cdot \\hat{b} = |\\hat{a}| * |\\hat{b}| * \\cos(\\alpha) =  1 * 1 * \\cos("
                             + str(round(alpha_tracker.get_value()))
                             + "^{\\circ}) = "
                             + str(round(np.cos(alpha_tracker.get_value() * DEGREES), 2)))
        ...
        vector_a.add_updater(
            lambda x: x.become(
                vector_a_copy.copy().rotate(alpha_tracker.get_value() * DEGREES, about_point=plane.get_origin())
            )
        )
        vector_a_label.add_updater(
            lambda x: x.become(
                vector_a_label_copy.copy()
                .rotate(alpha_tracker.get_value() * DEGREES, about_point=plane.get_origin())
                .rotate(-alpha_tracker.get_value() * DEGREES)  # Preserve orientation of the sign
            )
        )
        math_text.add_updater(
            lambda x: x.become(
                MathTex("\\hat{a} \\cdot \\hat{b} = |\\hat{a}| * |\\hat{b}| * \\cos(\\alpha) =  1 * 1 * \\cos("
                        + str(round(alpha_tracker.get_value()))
                        + "^{\\circ}) = "
                        + str(round(np.cos(alpha_tracker.get_value() * DEGREES), 2)))
                .next_to(plane, direction=UP, buff=2)
        ...
        self.play(alpha_tracker.animate().set_value(0.1))

        alpha = Angle.from_three_points(
        ...
        self.play(alpha_tracker.animate(run_time=2, rate_func=smoothererstep).set_value(90))
        self.wait(0.5)
        self.play(alpha_tracker.animate(run_time=2, rate_func=smoothererstep).set_value(180))
        self.wait(0.5)

        # A hack with value=0.1. Angles don't work if set to 0
        self.play(alpha_tracker.animate(run_time=3, rate_func=smoothererstep).set_value(0.1))
